...
def group_items(items: list[dict[str, Any]]) -> list[tuple[str, list[dict[str, Any]]]]:
    ...
    for item in items:
        section = str(item.get("section") or "Daily Shop")
...
def rarity_color(rarity: str) -> tuple[int, int, int]:
    value = rarity.lower()
    if "legendary" in value or "mythic" in value:
        return (255, 181, 54)
    if "epic" in value or "marvel" in value or "dc" in value:
        return (214, 93, 255)
    if "rare" in value:
        return (55, 153, 255)
...
def draw_card(
    ...
    draw.rounded_rectangle(card, radius=18, fill=CARD_BG, outline=LINE, width=2)
    draw.rounded_rectangle((x + 2, y + 2, x + CARD_WIDTH - 2, y + CARD_HEIGHT - 2), radius=16, outline=(255, 255, 255, 25), width=1)

    image_area = (x + 14, y + 54, x + CARD_WIDTH - 14, y + 54 + IMAGE_HEIGHT)
    draw.rounded_rectangle(image_area, radius=14, fill=CARD_BG_2)
    draw.line((image_area[0], image_area[3], image_area[2], image_area[3]), fill=(255, 255, 255, 30), width=1)

    section = str(item.get("section") or "Daily Shop")
    draw_badge(draw, (x + 14, y + 14), section, FONT_SMALL, (3, 12, 31), CARD_WIDTH - 28)
    ...
    draw_badge(draw, (x + 15, y + CARD_HEIGHT - 50), rarity, FONT_SMALL, rarity_color(rarity), CARD_WIDTH - 120)

    price_text = f"{int(item.get('price') or 0):,}"
    price_width, price_height = text_size(draw, price_text, FONT_PRICE)
    price_x = x + CARD_WIDTH - 16 - price_width
    icon_size = 28
    if vbuck_icon:
        price_x -= icon_size + 7
        icon = vbuck_icon.copy()
        icon.thumbnail((icon_size, icon_size), Image.Resampling.LANCZOS)
        base.alpha_composite(icon, (price_x, y + CARD_HEIGHT - 48))
        text_x = price_x + icon_size + 7
    else:
        text_x = price_x

    draw.text((text_x, y + CARD_HEIGHT - 50), price_text, fill=YELLOW, font=FONT_PRICE)
...
def draw_placeholder(path: Path) -> None:
    ...
    draw.text((PADDING, 210), "FORTNITE DAILY SHOP", fill=TEXT, font=FONT_TITLE)
    draw.text((PADDING, 286), "Run update_shop.py to generate the latest shop image.", fill=MUTED, font=FONT_SECTION)
...
def render_shop_image() -> None:
    ...
    draw.text((PADDING, PADDING + 82), f"Updated: {parse_date(data.get('updatedAt') or data.get('date'))}", fill=MUTED, font=FONT_META)
    draw.text((PADDING, PADDING + 112), "Auto generated from fortnite-api.com", fill=(123, 164, 213), font=FONT_SMALL)

    vbuck_icon = download_image(str(data.get("vbuckIcon") or ""))

    y = PADDING + HEADER_HEIGHT
    for section_name, section_items in groups:
        draw.rounded_rectangle((PADDING, y + 6, PADDING + 8, y + 42), radius=4, fill=YELLOW)
        draw.text((PADDING + 20, y), str(section_name), fill=TEXT, font=FONT_SECTION)
