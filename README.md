...
QQ `2353888741` 可以在群里艾特温德尔发送 `开始视奸` 或 `停止视奸`，全局开启或关闭 Steam 好友自动动态。关闭后仍会维护状态基线，重新开启不会补发已经在玩的好友；`Steam状态`、`Steam排行` 和每日排行榜不受影响。其他 QQ 号无权修改这个开关，开关状态会保存在 `bot_memory/steam_activity_settings.json`，重启后继续生效。
